...
const CityButton = () => {
    ...
    const { changeLocation, changeLink, isOpen } = useLocationModal();
        ...
        >
            <button className="text-black text-base flex justify-center items-center gap-1 py-4 max-w-[100px] max-h-[56px] sm:max-w-[200px] mx-1">
                ...
                <KeyboardArrowDownOutlinedIcon
                    fontSize="small"
                    className={`${cityHover && !isOpen ? "xl:rotate-180" : "xl:rotate-0 "} ${cityClick ? "rotate-180" : "rotate-0"
                        } transition duration-200`}
                />
            </button>
            <ul
                className={`${cityHover && !isOpen ? "xl:opacity-100 xl:scale-100" : "xl:opacity-0 xl:scale-0"
            ...
            >
                {cities.map((city) => (
                    <li
                        ...
                        className={`${splitedPathname[1] === city.capital && "underline"
