    ...
    <Fragment>
      <div className="max-w-[1300px] flex flex-col gap-4">
        {products?.map((product, index) => (
          <Fragment key={index}>
            {params.size && (
              <div className="relative w-full h-[400px] flex flex-col justify-center text-center sm:items-start px-6 xl:px-12">
                {" "}
                ...
                <div className="flex flex-col gap-6 max-w-[800px] justify-center sm:justify-start items-center sm:items-start">
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-0 justify-center sm:justify-start items-center w-full">
                    ...
                    <div className="w-4 h-1 sm:w-1 sm:h-4 sm:mx-3 sm:block bg-black/50"></div>
                    <h2 className="text-black/60 text-2xl underline">
                      {product.id.map((ids) =>
                        ids === decodeURI(params.id) ? ids : ""
                      )}
                    </h2>
                    <div className="w-4 h-1 sm:w-1 sm:h-4 sm:mx-3 bg-black/50 sm:block"></div>
                  ...
                  <p className="text-start text-lg hidden sm:block">
                    {product.description}
                  </p>
